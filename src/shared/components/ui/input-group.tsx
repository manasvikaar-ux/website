...
const InputGroup = ({ className, ...props }: ComponentProps<'fieldset'>) => (
  <fieldset
    className={cn(
...
const inputGroupAddonVariants = cva(
  "flex h-auto cursor-pointer select-none items-center justify-center gap-2 py-1.5 font-medium text-muted-foreground text-sm group-data-[disabled=true]/input-group:opacity-50 [&>kbd]:rounded-[calc(var(--radius)-5px)] [&>svg:not([class*='size-'])]:size-4",
  {
    defaultVariants: {
      align: 'inline-start',
    },
...
const InputGroupAddon = ({
  ...
  <button
    ...
    onClick={(e) => {
      // prevent focus hijack if clicking nested button
